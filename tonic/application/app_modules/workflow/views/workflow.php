					...
					<tbody id="workflow_list">
						<?php foreach ($workflows[$code] as $id => $workflow) : ?>
							<tr>
								...
								<td><input type="text" id="order_<?php echo $workflow['id']; ?>" name="order[<?php echo $workflow['id']; ?>]" value="<?php echo $workflow['order']; ?>" data-validate="required" data-type="text" title="<?php echo lang('workflow.order'); ?>" class="order"></td>
